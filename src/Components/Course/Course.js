...
const Course = () => {
  ...
  useEffect(() => {
    // In the fetchData async function in useEffect
    const fetchData = async () => {
      try {
        ...
        // Initialize testsResponse variable
        let testsResponse = { data: [] };
        
        // Try to fetch existing tests
        try {
          testsResponse = await axios.get(`${process.env.REACT_APP_HOST}/api/test/${playlistId}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
        ...
        if (playlistResponse.data.videos.length > 0) {
          ...
          for (let i = 0; i < videos.length; i += 5) {
            ...
            // Check if this section's test was already passed
            const isTestCompleted = testsResponse?.data?.some(
            ...
            sections.push({
              ...
              test: testsResponse?.data?.find(test => test.sectionIndex === sectionIndex),
              isLocked: sectionIndex > 0 && !testsResponse?.data?.some(
  ...
  // Function to generate MCQ questions based on video titles
  const generateTestForSection = async (sectionIndex) => {
    try {
      ...
      const prompt = `Create 10 multiple-choice questions (MCQs) based on the following video titles  for an e-learning platform :
...
Make the questions test understanding of the key concepts that would likely be covered in videos with these titles.`;
      ...
      if (response.data && response.data.content) {
        ...
        parsedQuestions = generateDefaultQuestions(10);
      }
      ...
      try {
        ...
        console.error('Error saving default test:', innerError);
